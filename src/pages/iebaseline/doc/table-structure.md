...
| `module_type` | character varying(50) | varchar | Optional module grouping such as `Site`, `Global`, or `Others`; `NULL` or blank values are displayed by the frontend as `Unspecified` |
...
- `module_master.module_type` is nullable and intentionally unconstrained so values can be populated manually.
...
Add the nullable module grouping column to an existing module master table:
